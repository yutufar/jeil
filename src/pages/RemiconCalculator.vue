  ...
  <div class="remcon-calc">
    <div class="calc-header">
      ...
      <div class="main-tabs">
        ...
        <button class="main-tab-btn" :class="{ active: mainTab === 'profit' }" @click="mainTab = 'profit'">💰 수익
          분석</button>
      </div>
    </div>
      ...
      <div class="input-panel">
          ...
          <div class="spec-grid">
            <div class="field">
              ...
              <div class="radio-group">
                <button v-for="s in slumps" :key="s" class="radio-btn" :class="{ active: selectedSlump === s }"
                  @click="selectedSlump = s">{{ s }}</button>
              </div>
            </div>
            <div class="field">
        ...
        <section class="input-section">
          ...
          <div class="dimension-inputs">
            <div class="dim-field">
              <label>가로</label>
              <input type="number" v-model.number="width" min="0" step="0.01" placeholder="0.00"
                @input="clampPositive('width')" />
              <span class="unit">m</span>
            </div>
            <div class="dim-sep">×</div>
            <div class="dim-field">
              <label>세로</label>
              <input type="number" v-model.number="length" min="0" step="0.01" placeholder="0.00"
                @input="clampPositive('length')" />
              <span class="unit">m</span>
            </div>
            <div class="dim-sep">×</div>
            <div class="dim-field">
              <label>높이</label>
              <input type="number" v-model.number="height" min="0" step="0.01" placeholder="0.00"
                @input="clampPositive('height')" />
              <span class="unit">m</span>
            </div>
    ...
    <div v-show="mainTab === 'profit'" class="calc-body">
      <div class="input-panel">
        ...
        <section class="input-section">
          ...
          <div class="tab-buttons">
            <button v-for="spec in specs" :key="spec.value" class="tab-btn" :class="{ active: pSpec === spec.value }"
              @click="pSpec = spec.value; calcProfit()">
              <span class="tab-label">{{ spec.label }}</span>
              <span class="tab-desc">{{ spec.desc }}</span>
            </button>
          </div>
          <div class="field" style="margin-top:4px">
            <label>슬럼프 (mm)</label>
            <div class="radio-group">
              <button v-for="s in slumps" :key="s" class="radio-btn" :class="{ active: pSlump === s }"
                @click="pSlump = s; calcProfit()">{{ s }}</button>
            </div>
          </div>
          <div class="field">
            <label>호칭강도 (MPa)</label>
            <div class="strength-groups">
              <div v-for="group in strengthGroups" :key="group.ratio" class="strength-group">
                <span class="group-ratio">배합비 {{ group.ratio }}</span>
                <div class="radio-group">
                  <button v-for="s in group.strengths" :key="s" class="radio-btn" :class="{ active: pStrength === s }"
                    @click="pStrength = s; calcProfit()">{{ s }}</button>
                </div>
              </div>
            </div>
          </div>
          <div class="field">
            ...
            <div class="rate-input-wrap">
              <div class="rate-slider-area">
                <input type="range" v-model.number="pRate" min="50" max="100" step="1" class="rate-slider"
                  @input="calcProfit" />
                <div class="rate-labels"><span>50%</span><span>100%</span></div>
              </div>
              <div class="rate-number-wrap">
        ...
        <div :class="{ disabled: !pStrength }">
          ...
          <div class="sim-section" :class="{ disabled: !pVolume }">
            ...
            <div class="result-card" style="margin-bottom:8px">
              ...
              <div class="card-value" style="color:#60a5fa">{{ fmt(pResult.simTotalIncVat) }}<span
                  class="card-unit">원</span></div>
            </div>
            ...
            <div class="result-card" style="border-color:#78350f;background:#1c100a;margin-bottom:8px">
              ...
              <div class="card-value" style="color:#f87171">{{ fmt(pResult.simTotalCost) }}<span
                  class="card-unit">원</span></div>
            </div>
...
const setDefaultUnitPrices = () => {
  profitCostItems.value.forEach(item => {
    switch (item.materialType) {
      case 'C1': item.unitPrice = 108     // 시멘트
        break
      case 'C2': item.unitPrice = 27      // 플라이애쉬 (수정됨)
        break
      case 'C3': item.unitPrice = 65      // 슬래그 (수정됨)
        break
      case 'G1': item.unitPrice = 18000   // 자갈
        break
      case 'S1': item.unitPrice = 22500   // 샌드밀
        break
      case 'AD1': item.unitPrice = 1000   // 혼화제
        break
    }
  })
}

const applyDefaultUsage = () => {
  profitCostItems.value.forEach(item => {
    if (item.usagePerM3 === null) {
      item.usagePerM3 = Number(item.defaultUsage)
    }
  })
}
...
const calcProfit = () => {
  ...
  const materialCost = profitCostItems.value.reduce((sum, item) => {
    const usage = item.usagePerM3 ?? Number(item.defaultUsage) ?? 0
    return sum + Math.round(item.unitPrice * usage)
  }, 0)
...
.price-table thead th.col-active {
  color: #93c5fd;
  background: #1e3a5f;
}
...
.profit-card.profit-pos {
  border-color: #064e3b;
  background: #0a2018;
}

.profit-card.profit-neg {
  border-color: #7f1d1d;
  background: #1a0a0a;
}
...
.profit-card.selling .pc-value {
  color: #60a5fa;
}

.profit-card.cost .pc-value {
  color: #f87171;
}

.profit-card.profit-pos .pc-value {
  color: #34d399;
}

.profit-card.profit-neg .pc-value {
  color: #f87171;
}
